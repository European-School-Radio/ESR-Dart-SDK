...
class ESRProductionsSharesCounterWebsocketService {
  ...
  bool _isConnected = false;
  WebSocketChannel? _channel;
  final StreamController<ESRProductionsSharesCounterByProductionResults> _controller =
  StreamController<ESRProductionsSharesCounterByProductionResults>.broadcast();

  ESRProductionsSharesCounterWebsocketService() {
    if (sdk.env == ESREnvironments.test) {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketTestUrl}/productions-shares/";
    } else {
      _baseWebSocketURL =
  ...
  void connect() {
    ...
    _channel?.stream.listen(
          (message) {
        Map<String, dynamic> jsonMessage = jsonDecode(message);
        _controller.add(ESRProductionsSharesCounterByProductionResults.fromJson(jsonMessage));
      },
      onError: (error) {
        _isConnected = false;
        disconnect();
  ...
  Stream<ESRProductionsSharesCounterByProductionResults> get stream => _controller.stream;

  StreamSubscription<ESRProductionsSharesCounterByProductionResults> addListener(
      void Function(ESRProductionsSharesCounterByProductionResults event) onData,
      {Function? onError,
        void Function()? onDone,
        bool? cancelOnError}) {
    return _controller.stream.listen(
...
class ESRProductionsFollowersWebsocketService {
  final sdk = ESRSDK();
  String _baseWebSocketURL = "";
  ESRLang? _language;
  int _pageSize = 1;
  ...
  StreamController<ESRProductionsFollowersByProductionResults>.broadcast();

  ESRProductionsFollowersWebsocketService() {
    if (sdk.env == ESREnvironments.test) {
      _baseWebSocketURL =
  ...
  void connect() {
    ...
    _channel?.stream.listen(
          (message) {
        Map<String, dynamic> jsonMessage = jsonDecode(message);
        _controller.add(ESRProductionsFollowersByProductionResults.fromJson(jsonMessage));
      },
      onError: (error) {
        _isConnected = false;
        disconnect();
  ...
  Stream<ESRProductionsFollowersByProductionResults> get stream => _controller.stream;

  StreamSubscription<ESRProductionsFollowersByProductionResults> addListener(
      void Function(ESRProductionsFollowersByProductionResults event) onData,
      {Function? onError,
        void Function()? onDone,
        bool? cancelOnError}) {
    return _controller.stream.listen(
...
class ESRProductionsBySchoolWebsocketService {
  final sdk = ESRSDK();
  String _baseWebSocketURL = "";
  ESRLang? _language;
  int _pageSize = 1;
  ...
  ESRProductionsBySchoolWebsocketService() {
    if (sdk.env == ESREnvironments.test) {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketTestUrl}/productions-by-school/";
    } else {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketUrl}/productions-by-school/";
    }
  }
  ...
  void connect() {
    ...
    urlBuilder.addQueryParam("sort", _sorting.value.toString());
    urlBuilder.addQueryParam("direction", _direction.value.toString());
    urlBuilder.addQueryParam("group_id", _groupId.toString());
...
class ESRProductionsByUserWebsocketService {
  final sdk = ESRSDK();
  String _baseWebSocketURL = "";
  ESRLang? _language;
  int _pageSize = 1;
  ...
  ESRProductionsByUserWebsocketService() {
    if (sdk.env == ESREnvironments.test) {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketTestUrl}/productions-by-user/";
    } else {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketUrl}/productions-by-user/";
    }
  }
