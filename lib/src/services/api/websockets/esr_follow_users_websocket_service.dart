...
class ESRFollowUsersByUserWebsocketService {
  final sdk = ESRSDK();
  ...
  int _page = 1;
  int? _userId;
  ESRFollowUsersSorting _sorting = ESRFollowUsersSorting.created;
  ESRSortingDirections _direction = ESRSortingDirections.desc;

  bool _isConnected = false;
  WebSocketChannel? _channel;
  final StreamController<ESRFollowUsersPaginatedResults> _controller =
  StreamController<ESRFollowUsersPaginatedResults>.broadcast();

  ESRFollowUsersByUserWebsocketService() {
    if (sdk.env == ESREnvironments.test) {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketTestUrl}/follow-users-by-user/";
    } else {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketUrl}/follow-users-by-user/";
    }
  }
  ...
  void setSorting(ESRFollowUsersSorting newSorting) {
    _sorting = newSorting;
  ...
  ESRFollowUsersSorting getSorting(){
    return _sorting;
  }
  ...
  void connect() {
    ...
    _channel?.stream.listen(
        ...
        _controller.add(ESRFollowUsersPaginatedResults.fromJson(jsonMessage));
      },
      onError: (error) {
        _isConnected = false;
        disconnect();
  ...
  Stream<ESRFollowUsersPaginatedResults> get stream => _controller.stream;

  StreamSubscription<ESRFollowUsersPaginatedResults> addListener(
      void Function(ESRFollowUsersPaginatedResults event) onData,
      {Function? onError,
...
class ESRFollowUsersByUserToWebsocketService {
  final sdk = ESRSDK();
  String _baseWebSocketURL = "";
  ESRLang? _language;
  int _pageSize = 1;
  ...
  ESRFollowUsersByUserToWebsocketService() {
    if (sdk.env == ESREnvironments.test) {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketTestUrl}/follow-users-by-user-to/";
    } else {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketUrl}/follow-users-by-user-to/";
    }
  }
  ...
  void connect() {
    ...
    if (_userToId != null){
      urlBuilder.addQueryParam("user_to_id", _userToId.toString());
    }
