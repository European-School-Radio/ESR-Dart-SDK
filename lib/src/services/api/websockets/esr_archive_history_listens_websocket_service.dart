...
import 'package:esr_dart_sdk/src/enums/sorting/esr_archive_history_listen_sorting.dart';
import 'package:esr_dart_sdk/src/global_parameters/server_config.dart';
import 'package:esr_dart_sdk/src/utils/url_builder.dart';
import 'package:web_socket_channel/web_socket_channel.dart';
import 'package:web_socket_channel/status.dart' as status;

class ESRArchiveHistoryListensByUserWebsocketService {
  final sdk = ESRSDK();
  ...
  int? _userId;
  String? _jwt;
  ESRArchiveHistoryListenSorting _sorting = ESRArchiveHistoryListenSorting.created;
  ESRSortingDirections _direction = ESRSortingDirections.desc;

  bool _isConnected = false;
  WebSocketChannel? _channel;
  final StreamController<ESRArchiveHistoryListensByUserResults> _controller =
  StreamController<ESRArchiveHistoryListensByUserResults>.broadcast();

  ESRArchiveHistoryListensByUserWebsocketService() {
    if (sdk.env == ESREnvironments.test) {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketTestUrl}/archive-history-listens-by-user/";
    } else {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketUrl}/archive-history-listens-by-user/";
    }
  }
  ...
  void setSorting(ESRArchiveHistoryListenSorting newSorting) {
    _sorting = newSorting;
  ...
  ESRArchiveHistoryListenSorting getSorting(){
    return _sorting;
  }
  ...
  void connect() {
    ...
    urlBuilder.addQueryParam("token", _jwt.toString());
    ...
    _channel?.stream.listen(
        ...
        _controller.add(ESRArchiveHistoryListensByUserResults.fromJson(jsonMessage));
      },
      onError: (error) {
        _isConnected = false;
        disconnect();
  ...
  Stream<ESRArchiveHistoryListensByUserResults> get stream => _controller.stream;

  StreamSubscription<ESRArchiveHistoryListensByUserResults> addListener(
      void Function(ESRArchiveHistoryListensByUserResults event) onData,
      {Function? onError,
