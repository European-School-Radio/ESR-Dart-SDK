...
import 'package:esr_dart_sdk/src/enums/sorting/esr_user_sorting.dart';
import 'package:esr_dart_sdk/src/global_parameters/server_config.dart';
import 'package:esr_dart_sdk/src/utils/url_builder.dart';
import 'package:web_socket_channel/web_socket_channel.dart';
import 'package:web_socket_channel/status.dart' as status;

class ESRUsersSearchWebsocketService {
  final sdk = ESRSDK();
  ...
  final StreamController<ESRUsersWebsocketListResults> _controller =
  StreamController<ESRUsersWebsocketListResults>.broadcast();

  ESRUsersSearchWebsocketService() {
    if (sdk.env == ESREnvironments.test) {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketTestUrl}/users/";
    } else {
      _baseWebSocketURL =
      "${ESRServerConfig.websocketUrl}/users/";
    }
  }
  ...
  void setSorting(ESRUserSorting newSorting) {
    _sorting = newSorting;
  ...
  void connect() {
    ...
    _channel?.stream.listen(
        ...
        _controller.add(ESRUsersWebsocketListResults.fromJson(jsonMessage, _pageSize));
      },
      onError: (error) {
        _isConnected = false;
        disconnect();
  ...
  Stream<ESRUsersWebsocketListResults> get stream => _controller.stream;

  StreamSubscription<ESRUsersWebsocketListResults> addListener(
      void Function(ESRUsersWebsocketListResults event) onData,
      {Function? onError,
