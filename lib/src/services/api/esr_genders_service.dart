...
import 'package:esr_dart_sdk/src/enums/sorting/esr_gender_sorting.dart';
import 'package:esr_dart_sdk/src/global_parameters/server_config.dart';
import 'package:esr_dart_sdk/src/utils/url_builder.dart';
import 'package:http/http.dart' as http;

class ESRGendersService {
  final sdk = ESRSDK();
  String _apiURL = "";
  ...
  Future<ESRGendersPaginatedResults> getAllGenders({
    int? page,
    int? limit,
    ESRLang? language,
    ESRGenderSorting? sorting,
    ...
    if (response.statusCode == 200) {
      ...
      return ESRGendersPaginatedResults.fromJson(jsonData, limit);
    } else {
  ...
  Future<ESRGender> getGenderById(int id, {ESRLang? language}) async {
    ...
    if (response.statusCode == 200) {
      var responsePlain = await response.stream.bytesToString();
      var jsonData = json.decode(responsePlain);
      return ESRGender.fromJson(jsonData['gender']);
    } else if (response.statusCode == 404){
      throw ObjectNotFoundException("Gender with id $id not found");
    } else {
      throw HttpRequestNotSucceededException(response.reasonPhrase ?? "HTTP Request not Succeeded");
    }
  }
