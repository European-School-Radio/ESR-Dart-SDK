...
import 'package:esr_dart_sdk/src/enums/sorting/esr_sector_sorting.dart';
import 'package:esr_dart_sdk/src/global_parameters/server_config.dart';
import 'package:esr_dart_sdk/src/utils/url_builder.dart';
import 'package:http/http.dart' as http;

class ESRSectorsService {
  final sdk = ESRSDK();
  String _apiURL = "";
  ...
  Future<ESRSectorsPaginatedResults> getAllSectors({
    int? page,
    int? limit,
    ESRLang? language,
    ESRSectorSorting? sorting,
    ...
    if (response.statusCode == 200) {
      ...
      return ESRSectorsPaginatedResults.fromJson(jsonData, limit);
    } else {
  ...
  Future<ESRSector> getSectorById(int id, {ESRLang? language}) async {
    ...
    if (response.statusCode == 200) {
      var responsePlain = await response.stream.bytesToString();
      var jsonData = json.decode(responsePlain);
      return ESRSector.fromJson(jsonData['sector']);
    } else if (response.statusCode == 404){
      throw ObjectNotFoundException("Sector with id $id not found");
    } else {
      throw HttpRequestNotSucceededException(response.reasonPhrase ?? "HTTP Request not Succeeded");
    }
  }
